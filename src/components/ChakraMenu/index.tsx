                ...
                <div className='pb-5 border-b border-zinc-300'>
                    <h1 className='text-xl sm:text-lg font-bold text-zinc-800 mb-2'>Core Courses</h1>
                    {
                        CompulsoryData.map((val, index) => {
                            ...
                            return (
                                <Link onClick={() => setOpen(false)} key={index} href={`/compulsory/${val.id}`}><MenuItem className={`text-zinc-600 sm:text-sm ${active ? 'bg-zinc-200' : 'hover:text-main'} pl-2 py-[5px]`}>{val.text}</MenuItem></Link>
                            )
                        })
                    }
                </div>
                <div className='mt-5'>
                    <h1 className='text-xl sm:text-lg font-bold text-zinc-800 mb-2'>Advance Courses</h1>
                    {
                        tracks.map((val, index) => {
                            ...
                            return (
                                <Link onClick={() => setOpen(false)} key={index} href={`/tracks/${val.id}/4`}><MenuItem className={`text-zinc-600 sm:text-sm ${active ? 'bg-zinc-200' : 'hover:text-main'} pl-2 py-[5px]`}>{val.name}</MenuItem></Link>
                            )
                        })
                    }
                </div>
